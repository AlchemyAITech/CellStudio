...
class Evaluator:
    ...
    def _save_predictions(
        ...
        """Serialize predictions to a pickle file and UDF format.
        ...
        try:
            ...
            # Universal Data Foundation JSON Dump
            from .udf_hook import UDFSerializer
            UDFSerializer.dump_predictions_to_udf(
                work_dir=work_dir,
                y_true=y_true,
                y_pred=y_pred,
                y_prob=y_prob,
            ...
            # Legacy pickle dump
            if isinstance(self._predictions[0], list) or 'CellStudioInferResult' in type(self._predictions[0]).__name__:
                return  # Skip dense pickle dumping for hybrid segmentations
