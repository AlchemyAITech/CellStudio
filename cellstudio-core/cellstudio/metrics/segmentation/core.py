                ...
                for sample in batch:
                    ...
                    if hasattr(sample, 'gt_instance_seg'):
                        ...
                        if isinstance(masks, list):
                            import cv2
                            shape = getattr(sample, 'img_shape', (512, 512))
                            h, w = shape[:2]
                            inst_map = np.zeros((h, w), dtype=np.int32)
                            for i, m_poly in enumerate(masks):
                                poly = np.array(m_poly, dtype=np.int32)
                                cv2.fillPoly(inst_map, [poly], i + 1)
                            flat_gt.append(inst_map)
                        else:
                ...
                for res in batch:
                    if hasattr(res, 'masks') and res.masks is not None:
                        ...
                        if isinstance(masks, list):
                            ...
                            # Ensure Prediction Map Shape exactly matches Ground Truth Shape
                            if len(flat_gt) > len(flat_pred) and flat_gt[len(flat_pred)] is not None:
                                gt_shape = flat_gt[len(flat_pred)].shape
                                if inst_map.shape != gt_shape:
                                    import cv2
                                    inst_map = cv2.resize(inst_map, (gt_shape[1], gt_shape[0]), interpolation=cv2.INTER_NEAREST)
