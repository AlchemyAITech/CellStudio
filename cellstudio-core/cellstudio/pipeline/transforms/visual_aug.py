            ...
            if 'gt_bboxes' in results and len(results['gt_bboxes']) > 0:
                ...
                if 'gt_masks' in results and len(results['gt_masks']) > 0:
                    ...
                    if isinstance(masks, list):
                        flipped_masks = []
                        valid_mask_list = valid_mask.tolist() if isinstance(valid_mask, np.ndarray) else valid_mask
                        for i, m in enumerate(masks):
                            if valid_mask_list[i]:
                                m_array = np.array(m, dtype=np.float32)
                                if m_array.size == 0 or m_array.shape[-1] != 2:
                                    flipped_masks.append(m)
                                    continue
                                if self.direction == 'horizontal':
                                    m_array[..., 0] = w - m_array[..., 0]
                                else:
                                    m_array[..., 1] = h - m_array[..., 1]
                                flipped_masks.append(m_array.tolist())
                        results['gt_masks'] = flipped_masks
                    else:
        ...
        if 'img' in results:
            ...
            if 'gt_bboxes' in results and len(results['gt_bboxes']) > 0:
                ...
                if 'gt_masks' in results and len(results['gt_masks']) > 0:
                    masks = results['gt_masks']
                    if isinstance(masks, list):
                        # It is a list of polygon coordinates
                        resized_masks = []
                        valid_mask_list = valid_mask.tolist() if isinstance(valid_mask, np.ndarray) else valid_mask
                        for i, m in enumerate(masks):
                            if valid_mask_list[i]:
                                m_array = np.array(m, dtype=np.float32)
                                if m_array.size == 0 or m_array.shape[-1] != 2:
                                    resized_masks.append(m) # fallback unmodified
                                    continue
                                m_array[..., 0] *= w_scale
                                m_array[..., 1] *= h_scale
                                resized_masks.append(m_array.tolist())
