...
### 2.1 基础图元实体边界与结构测试 (Primitive Objects & Hierarchy)
确保脱离了模型本身的基类具有极强的自校验与反序列化解析能力。
- [ ] **空间基元拉伸**：对 `CSUPoint`, `CSUBBox`, `CSUPolygon`, `CSUMask` 强制输入非法值（如越界负数、塌缩的零面积框），验证系统抛出的防护级别而非静默报错。
- [ ] **层级树嵌套 (Annotation Hierarchy) 测试**：验证 `CSUFeatureCollection` 中的 Parent-Child 映射是否生效。比如一个肿瘤 `Parent BBox` 销毁时，它所包含的所有细胞 `Children Polygons` 是否被级联安全摘除。
- [ ] **坐标系降维验证 (Level-0 Consistency)**：模拟从 Level-3 (低配) 加载图像计算多边形后，反向映射保存，验证写回 UDF 的坐标点阵是否以严格的物理大图 Level-0 `1:1` 被还原，没有浮点舍入导致的撕裂。
...
### 2.4 纯病理图像级特性：WSI 与数据增强引擎 (WSI & Data Augmentations) 可视化打靶
不仅保证代码不报错，更保证变换空间里的代数几何转换具有视觉合法性。
- [ ] **WSI 瓦块切片与 Target MPP 重采样测试**：
  - 测试读取极高倍率原生大图（如 0.25 MPP），测试程序在提取瓦块并重置为统一标准（如设定为 `0.5 MPP`）时，**细胞等比例缩放倍率正常**。
  - **滑窗连通性拼缝**：对超大图按步长 (`stride`) 与固定 Overlap 裁切导出后，编写脚本将瓦块物理拼合，肉眼检查接缝处的病灶多边形是否完美闭合。
- [ ] **病理专属增广 (PathologyExpandRotation) 验证**：
  - 使用 `tools/debug_visual_aug.py`，启动带有延展参数的大角度旋转。验证边框不是填黑补缺（会导致病变区域被斩尾），而是从全景层取周围组织结构进行补齐计算，实现旋转前后的背景自然融汇。
- [ ] **推理去重：基于中心点 (Centroid) 的切片融合过滤查验机制**：
  - 启动带 Padding 冗余的瓦片预测流，收集极高密度的推流结果。
  - **核心验证点**：严格验证那些横跨两个瓦片切割边界的细胞预测特征。必须确保中心点落入冗余 Margin 的特征被静默丢弃，而中心点落入 Core 区的被保留，不得出现因浮点截断导致两边都丢失或都保留的双轨错误。
